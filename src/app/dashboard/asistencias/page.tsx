...
import { calcularPorcentajeLocal, getAsistenciaColor, formatFecha, cn } from '@/lib/utils'
...
export default function AsistenciasPage() {
  ...
  const handleEstadoChange = async (id: string, nuevoEstado: 'PRESENTE' | 'AUSENTE' | 'JUSTIFICADO') => {
    setUpdatingId(id)
    try {
        ...
        <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
          ...
          {totalDate > 0 && (
            <span className="text-xs text-neutral-400">
              {presentes}/{totalDate} presentes
            </span>
          )}
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-sm" aria-label="Tabla de asistencias">
            <thead>
                          ...
                          <td className="px-5 py-3 hidden md:table-cell">
                            {porcentaje !== null ? (
                              <div className="flex items-center gap-2">
                                ...
                                {porcentaje < 75 && (
                                  <Warning size={14} className="text-red-500" weight="fill" aria-label="Porcentaje crítico" />
                                )}
                              </div>
                            ) : '—'}
                          </td>
                            ...
                            <div className="flex items-center justify-end gap-1">
                              {ESTADOS.map((estado) => (
                                <button
                                  ...
                                  aria-label={`Marcar como ${estado.toLowerCase()}`}
                                >
